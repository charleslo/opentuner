...
class ConfigurationManipulatorBase(object, metaclass=abc.ABCMeta):
  """
...
class ConfigurationManipulator(ConfigurationManipulatorBase):
  ...
  def hash_config(self, config):
    ...
    for i, p in enumerate(params):
      ...
      m.update(p.hash_value(config).encode('utf-8'))
...
class PrimitiveParameter(Parameter, metaclass=abc.ABCMeta):
  """
  ...
  def hash_value(self, config):
    ...
    return hashlib.sha256(repr(self.get_value(config)).encode('utf-8')).hexdigest()
...
class ComplexParameter(Parameter):
  ...
  def hash_value(self, config):
    """produce unique hash for this value in the config"""
    self.normalize(config)
    return hashlib.sha256(repr(self._get(config)).encode('utf-8')).hexdigest()
